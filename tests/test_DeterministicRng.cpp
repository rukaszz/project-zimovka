...
TEST(DeterministicRngTest, KnownValue_Seed0_FirstOutput){
    ...
    // std::mt19937 seed 0 の初期値 = 2357136044
 ...
 *
 * DeterministicRngがmt19937を正確にラップしていること，
 * それによって余分な消費・スキップが起きていないことを検証する
 */
...
/**
 * @brief 別のシード値をDeterministicRngと独立したstd::mt19937に与え，
 * 出力が一致することを確認
 * 
 */
 ...
 * →リプレイ用：乱数消費後にReseedすれば再び同一列が得られる
 * 
 */
 ...
 *
 * range=1なので棄却サンプリングが起きずoffset=0となりminが返る
 */
 ...
 *
 * 実装は(NextU32() >> 8) / 2^24なので，
 * 最小値=0.0f, 最大値=(2^24-1)/2^24 であり，1.0f未満であることを保証できる
 */
...
/**
 * @brief UnitFloat()が負の値を返さないことを確認
 * 
 */
 ...
 *
 * 最大値は (2^24 - 1) / 2^24 ≈ 0.99999994f であり，1.0fを超えない実装になっている
 */
